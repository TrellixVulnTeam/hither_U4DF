...
export default class ESSTaskRegulator {
    constructor(config) {
        this._channels = {};
    ...
    approveTask(taskName, streamId, numBytes, channelName, signature, req) {
        ...
        else if (taskName === 'getNumEvents') {
            return channel.approveGetNumEventsTask(numBytes, req);
        }
        else {
    ...
    finalizeTask(taskName, channelName, numBytes, approvalObject) {
        ...
        if (taskName === 'readEvents') {
            ...
            return channel.finalizeGetNumEventsTask(approvalObject);
        }
        else {
...
class Channel {
    ...
    approveGetNumEventsTask(numBytes, req) {
        return { approve: true };
    }
    ...
    finalizeGetNumEventsTask(numBytes, approvalObject) {
    }
}
...
function verifySignature(taskName, streamId, password, signature) {
    if (process.env.EVENT_STREAM_SERVER_TEST_SIGNATURE) {
        if ((signature === process.env.EVENT_STREAM_SERVER_TEST_SIGNATURE)) {
            console.warn('WARNING: verified using test signature from EVENT_STREAM_SERVER_TEST_SIGNATURE environment variable');
            return true;
        }
    }
